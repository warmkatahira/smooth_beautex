    ...
    public function getCreateItem()
    {
        ...
        $items = Item::joinSub($order_quantities, 'order_quantities', function ($join) {
                        ...
                        ->joinSub($shipping_group_shipping_quantity, 'shipping_group_shipping_quantity', function ($join) {
                            $join->on('stocks.item_id', '=', 'shipping_group_shipping_quantity.item_id')
                                ->on('stocks.base_id', '=', 'shipping_group_shipping_quantity.shipping_base_id');
                        })
                        ->select(
                            'items.item_code',
                            'items.item_jan_code',
                            ...
                            DB::raw("stocks.total_stock - COALESCE(shipping_group_shipping_quantity.shipping_group_shipping_quantity, 0) as remaining_stock")
                        )
                        ->groupBy('items.item_code', 'items.item_jan_code', 'items.item_name', 'stocks.item_location', 'stocks.total_stock', 'total_shipping_quantity', 'shipping_group_shipping_quantity.shipping_group_shipping_quantity')
