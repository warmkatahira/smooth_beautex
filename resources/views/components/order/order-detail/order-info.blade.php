    ...
    <div class="flex flex-row gap-5">
        ...
        <div class="w-1/2">
            <div class="flex flex-col">
                ...
                <x-order.order-detail.info-div label="注文日時" :value="CarbonImmutable::parse($order->order_date . ' ' . $order->order_time)->isoFormat('Y年MM月DD日(ddd) HH:mm:ss')" />
                <x-order.order-detail.info-div label="受注管理ID" :value="$order->order_control_id" />
                <x-order.order-detail.info-div label="モール" :value="$order->order_category->mall->mall_name" />
            </div>
        </div>
    </div>
</div>