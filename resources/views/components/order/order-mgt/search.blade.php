<form method="GET" action="{{ route($route) }}" id="search_form">
    ...
    <div class="flex flex-col gap-y-2 p-3 bg-white min-w-60 text-xs border border-black">
        ...
        <x-search.input type="date" label="配送希望日" id="search_desired_delivery_date" />
        <input type="hidden" id="order_status_id" name="order_status_id" value="{{ session('search_order_status_id') }}">
