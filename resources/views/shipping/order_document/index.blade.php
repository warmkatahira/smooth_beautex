        ...
        <tbody class="bg-white">
            @foreach($ranges as $range)
                <tr class="text-left hover:bg-theme-sub cursor-default whitespace-nowrap">
                    <td class="py-1 px-2 border text-center"><img src="{{ asset('image/'.$shipping_method->delivery_company->company_image) }}" class="inline-block"></td>
                    <td class="py-1 px-2 border text-center">{{ $shipping_method->shipping_method }}</td>
                    ...
                    <td class="py-1 px-2 border">
                        <a href="{{ route('kobetsu_picking_list_create.create', ['shipping_method_id' => $range['shipping_method_id'], 'start' => $range['start'], 'end' => $range['end']]) }}" class="btn bg-btn-enter text-white py-1 px-2" target="_blank">個別ピッキングリスト</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
